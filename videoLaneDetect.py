...
fileSize = (1280,720) # Assumes 1920x1080 mp4
scaleRatio = 1 # Option to scale to fraction of original size. 
...
class videoLaneDetect:
  ...
  def __init__(self, origFrame):
    ...
    #   (600,470), # Top-left corner
      (570,470), # Top-left corner
      (270,684), # Bottom-left corner
      (1100,680), # Bottom-right corner
  ...
  def calculateHistogram(self,frame=None,plot=True):
  ...
  def getLaneLinePreviousWindow(self, leftFit, rightFit, plot=False):
    ...
    prevLeftFit2.append(leftFit)
    prevRightFit2.append(rightFit)

    # Calculate the moving average	
    if len(prevLeftFit2) > 10:
      prevLeftFit2.pop(0)
      prevRightFit2.pop(0)
      leftFit = sum(prevLeftFit2) / len(prevLeftFit2)
      rightFit = sum(prevRightFit2) / len(prevRightFit2)
  ...
  def getLaneLineIndicesSlidingWindows(self, plot=False):
    ...
    for window in range(noOfWindows):
      # Find window boundaries in x and y and right and left
    ...
    # There must be some nonzero pixels		
    if len(leftx)==0 or len(lefty)==0 or len(rightx)==0 or len(righty)==0:
      leftx = prevLeftx
      lefty = prevLefty
      rightx = prevRightx
    ...
    prevLeftFit.append(leftFit)
    prevRightFit.append(rightFit)
	
    if len(prevLeftFit) > 10:
      prevLeftFit.pop(0)
      prevRightFit.pop(0)
      leftFit = sum(prevLeftFit) / len(prevLeftFit)
      rightFit = sum(prevRightFit) / len(prevRightFit)
  ...
  def perspectiveTransform(self, frame=None, plot=False):
    ...
    (thresh, binary_warped) = cv2.threshold(self.warpedFrame, 127, 255, cv2.THRESH_BINARY)			
    self.warpedFrame = binary_warped
...
def main():
  ...
  result = cv2.VideoWriter(outputFilepath + '\\output_' + filename.group(1)[1:], fourcc, 20.0, fileSize) 
  ...
  while cap.isOpened():
    ...
    if success:
      ...
      width = int(frame.shape[1] * scaleRatio)
      height = int(frame.shape[0] * scaleRatio)
      frame = cv2.resize(frame, (width, height))

      originalFrame = frame.copy()

      laneObj = videoLaneDetect(origFrame=originalFrame)
      ...
      frameWithLaneLines2 = laneObj.displayCurvatureOffset(frame=frameWithLaneLines, plot=debugMode)

      result.write(frameWithLaneLines2)
			
      cv2.imshow("Frame", frameWithLaneLines2) 	
